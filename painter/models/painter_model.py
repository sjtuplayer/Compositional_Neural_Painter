...
class PainterModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.model_names = ['g']
        self.controler_paras_num=max(((opt.controler_points_w+opt.controler_points_h)*2-4)*2,0)
        self.d = 12+self.controler_paras_num  # xc, yc, w, h, theta, R0, G0, B0, R2, G2, B2, A
        self.d_shape = 5+self.controler_paras_num
        self.d_shape0=5
        self.tps=util.tps(opt.controler_points_w,opt.controler_points_h,self.device)
        def read_img(img_path, img_type='RGB'):
            ...
            return img
        if self.controler_paras_num!=0:
            if self.opt.use_small_brush:
                brush_large_vertical = read_img('brush/brush_large_vertical_pad_small.png', 'L').to(self.device)
                brush_large_horizontal = read_img('brush/brush_large_horizontal_pad_small.png', 'L').to(self.device)
            else:
                brush_large_vertical = read_img('brush/brush_large_vertical_pad.png', 'L').to(self.device)
                brush_large_horizontal = read_img('brush/brush_large_horizontal_pad.png', 'L').to(self.device)
        else:
            print('load_ori_img')
        ...
        net_g = networks.Painter(self.d_shape, opt.used_strokes, opt.ngf,
                                 n_enc_layers=opt.num_blocks, n_dec_layers=opt.num_blocks,patch_size=opt.patch_size)
        ...
        self.resize = transforms.Resize((self.patch_size,self.patch_size))
        ...
        if opt.G_render:
            self.G = networks.ZouFCNFusion()
            checkpoint = torch.load(os.path.join('pretrained_models', 'oil_brush.pt'),
                                    map_location=None if self.device == 'cuda' else self.device)
    ...
    def param2stroke(self, param, H, W):
        ...
        R0, G0, B0, R2, G2, B2, _ = param_list[-7:]
        if self.controler_paras_num!=0:
            controller_paras=[item for item in param_list[5:-7]]
        ...
        #brush0=brush.contiguous()
        if self.controler_paras_num!=0:
            brush=self.tps.forward(brush,controller_paras)
            # tmp=brush-brush0
    ...
    def param2stroke_G(self, param):
        v=param.unsqueeze(-1).unsqueeze(-1)
        foregrounds, alphas = self.G(v)
        foregrounds = morphology.Dilation2d(m=1)(foregrounds)
        alphas = morphology.Erosion2d(m=1)(alphas)
        foregrounds=self.resize(foregrounds)
        alphas=self.resize(alphas)
        return foregrounds,alphas
    def set_input(self, input_dict):
        self.image_paths = input_dict['A_paths']
        with torch.no_grad():
            old_used_strokes=random.randint(1,50)
            old_param = torch.rand(self.opt.batch_size // 4, old_used_strokes, self.d, device=self.device)
            old_param[:, :, :4] = old_param[:, :, :4] * 0.5 + 0.2
            if self.controler_paras_num!=0:
                old_param[:, :, 2:4] = old_param[:, :, 2:4]*2
                old_param[:,:,5:5+self.controler_paras_num]=(old_param[:,:,5:5+self.controler_paras_num]-0.5)*0.4
            #old_param[:, :, 5:5 + self.controler_paras_num] = old_param[:, :,5:5 + self.controler_paras_num] *0
            ...
            foregrounds = morphology.Dilation2d(m=1)(foregrounds)
            alphas = morphology.Erosion2d(m=1)(alphas)
            foregrounds = foregrounds.view(self.opt.batch_size // 4, old_used_strokes, 3, self.patch_size * 2,
                                           self.patch_size * 2).contiguous()
            alphas = alphas.view(self.opt.batch_size // 4, old_used_strokes, 3, self.patch_size * 2,
                                 self.patch_size * 2).contiguous()
            old = torch.zeros(self.opt.batch_size // 4, 3, self.patch_size * 2, self.patch_size * 2, device=self.device)
            for i in range(old_used_strokes):
            ...
            if self.controler_paras_num != 0:
                gt_param[:, :, 2:4] = gt_param[:, :, 2:4]*2
                gt_param[:, :, 5:5 + self.controler_paras_num] = (gt_param[:, :,
                                                                   5:5 + self.controler_paras_num] - 0.5) * 0.4
            #gt_param[:, :, 5:5 + self.controler_paras_num] = gt_param[:, :, 5:5 + self.controler_paras_num] * 0
    ...
    def optimize_parameters(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.gt_param.shape[0]):
                ...
                pred_param_broad = self.pred_param[i,:,:self.d_shape0].unsqueeze(1).contiguous().repeat(
                    1, valid_gt_param.shape[0], 1)
                valid_gt_param_broad = valid_gt_param[:,:self.d_shape0].unsqueeze(0).contiguous().repeat(
        ...
        self.loss_w = self.gaussian_w_distance(paired_pred_param[:,:self.d_shape0], paired_gt_param[:,:self.d_shape0]).mean() * self.opt.lambda_w
