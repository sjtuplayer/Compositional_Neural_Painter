...
#writer = TensorBoard('../train_log2/{}'.format(exp))
writer=None
        ...
        if (episode_steps >= max_step and max_step):
            if step > args.warmup:
                ...
                if episode > 0 and validate_interval > 0 and episode % validate_interval == 0:
                    ...
                    if debug: prRed('Step_{:07d}: mean_reward:{:.5f} mean_dist:{:.5f} var_dist:{:.5f}'.format(step - 1, np.mean(reward), np.mean(dist), np.var(dist)))
                    with open(args.log_dir+'/test_loss.txt','a') as f:
                        f.write('step:%d,   reward:%f,   l2_loss:%f\n'%(step,float(np.mean(reward)),float(np.mean(dist))))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--validate_episodes', default=1, type=int,
    ...
    parser.add_argument('--episode_train_times', default=50, type=int)
    parser.add_argument('--resume', default=None,type=str)
    parser.add_argument('--output', default='./model3', type=str)
    parser.add_argument('--log_dir', default='./train_log3', type=str)
    parser.add_argument('--debug', dest='debug',default=True, action='store_true')
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument('--param_num', default=5, type=int)
    parser.add_argument('--no_step', action='store_true', default=True)
    parser.add_argument('--stroke_time', default=1, type=int)
    ...
    fenv = fastenv(args.max_step, args.env_batch,args.log_dir,args.dataset_path,writer)
    agent = DDPG(args,args.batch_size, args.env_batch, args.max_step, \
                 args.tau, args.discount, args.rmsize, \
                 writer, args.resume, args.output)
    evaluate = Evaluator(args, writer)
    # agent.test()
    # exit()
    with open(os.path.join(args.log_dir,'train_loss.txt'),'w')as f:
        pass
    with open(os.path.join(args.log_dir,'test_loss.txt'),'w')as f:
        pass
